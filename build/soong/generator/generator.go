...
func init() {
	android.RegisterModuleType("dora_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = doraExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = doraExpandVariables(ctx, dep_file)
	...
	cmd := doraExpandVariables(ctx, String(g.properties.Cmd))
